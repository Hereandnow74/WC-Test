...
<p>Capturing a Tier 1-10 target adds 60% of the list price for their effective tier after step 1a to your account, rounded down to the nearest whole number. Capture credits are not granted for local versions of targets that you’ve purchased or alternate versions that you’ve already captured once, unless they’re a unique entry in the lists proper or as a waifu perk. If a target is only listed as part of a group entry, you will only get the credits after capturing every member of the group.</p>
...
<p>If you only wanted a capture for their credit value, you may ship them to us at the company for 20% of the list price for their effective tier after step 4, again rounded down to the nearest whole number. A target’s capture and sale values combined cannot be higher than the list price for their effective tier after step 4. Humanoid familiars gained for free from an Origin choice will not count for capture credits; their sale value is still 20% of the list price for their effective tier after step 4, rounded down to the nearest whole number. If a subject’s capture value is zero, their sale value will also be zero. Selling a subject only takes a single button press in the company app on your smart device. We’ll reprocess the captures and send them where they need to go. Otherwise, captures are yours to keep.</p>
`
