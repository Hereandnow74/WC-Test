  ...
  >
    <div class="h-full flex flex-col" :class="[isUserChar ? 'bg-lime-50 dark:bg-warm-gray-800': 'bg-gray-300 dark:bg-gray-800']">
      ...
      <div class="py-1 h-max">
        ...
        <div v-if="waifuPerksObject[char.cs]" class="ml-2 my-1 text-xs text-gray-600 dark:text-gray-400 leading-none">
          SWP: <router-link class="font-bold hover:underline" :to="{path: '/talents/specific', hash: `#${waifuPerksObject[char.cs].title}`}">
            {{ waifuPerksObject[char.cs].title }}
          </router-link>
        </div>
...
import { CHAR_COSTS, defTags, PLACEHOLDER_NO_IMAGE, useAllChars, waifusThatHasPerk, waifuTags, CHAR_COSTS_TICKET } from '~/data/constants'
...
import { captureCopyCompanion, captureCompanion, deleteLocalCharacter, undoBuying, isAlreadyBought } from '~/logic/waifuLogic'
